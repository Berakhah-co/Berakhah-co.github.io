...
def agregar_producto():
    ...
    titulo = soup.new_tag('h3', **{'class': 'titulo-producto', 'onclick': f"enviarWhatsApp('{nombre_completo_producto}')"})
...
# Botones de acciones en el panel izquierdo
tk.Button(left_frame, text="Cargar Archivo HTML", command=seleccionar_archivo, width=25, height=2, bg='#ad8a1f', fg='white').pack(pady=5)
tk.Button(left_frame, text="Agregar Producto", command=agregar_producto, width=25, height=2, bg='#ad8a1f', fg='white').pack(pady=5)
tk.Button(left_frame, text="Eliminar Producto", command=eliminar_producto, width=25, height=2, bg='#F44336', fg='white').pack(pady=5)
tk.Button(left_frame, text="Ocultar Producto", command=ocultar_producto, width=25, height=2, bg='#F44336', fg='white').pack(pady=5)
tk.Button(left_frame, text="Mostrar Producto", command=mostrar_producto, width=25, height=2, bg='#ad8a1f', fg='white').pack(pady=5)
tk.Checkbutton(left_frame,text="Habilitar Reordenar",variable=reordenar_activo, bg='#ad8a1f',  fg='#FFFFFF',  selectcolor='#333333', width=22,  height=2,).pack(pady=5) # Checkbutton para habilitar/deshabilitar reordenar
tk.Button(left_frame, text="Guardar Cambios", command=guardar_cambios, width=25, height=2, bg='#4CAF50', fg='white').pack(pady=5)
tk.Button(left_frame, text="Git and page", command=open_web_page, width=25, height=2, bg='#2196F3', fg='white').pack(pady=0)
# Ruta del logo
logo_path = os.path.join(os.getcwd(), 'Logo.jpg')
# Cargar la imagen del logo
logo_img = Image.open(logo_path)
# Redimensionar la imagen si es necesario (ajusta el tamaño)
logo_img = logo_img.resize((130, 140), Image.LANCZOS)  # Ajusta las dimensiones (150x150 es un ejemplo)
# Convertir la imagen a un formato compatible con Tkinter
logo_img_tk = ImageTk.PhotoImage(logo_img)
# Crear un label para mostrar la imagen del logo
logo_label = tk.Label(left_frame, image=logo_img_tk, bg='#000000') 
# Empaquetar el logo debajo de los botones
...
tk.Button(boton_frame, text="Seleccionar Imágenes", command=seleccionar_imagenes, bg='#4CAF50', fg='white').pack(side=tk.LEFT, padx=5)

# Botón para limpiar campos al lado del botón seleccionar imágenes
tk.Button(boton_frame, text="Limpiar Campos", command=limpiar_campos, bg='#4CAF50', fg='white').pack(side=tk.LEFT, padx=5)
